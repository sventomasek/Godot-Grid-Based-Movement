...
func _ready():
	...
	newPosition = tileMapLayer1.map_to_local(currentTile)
...
func _physics_process(delta):
	...
	if (player.moved && nextDir == "x") || !moveWithPlayer:
		...
		if changeXDirWhen == "Touches Wall":
			var leftTileData: TileData = tileMapLayer1.get_cell_tile_data(tileMapLayer1.local_to_map(global_position) + Vector2i(-1, 0))
			var rightTileData: TileData = tileMapLayer1.get_cell_tile_data(tileMapLayer1.local_to_map(global_position) + Vector2i(1, 0))
	...
	if movementType == "Platformer" && ((player.moved && nextDir == "y") || !moveWithPlayer):
		# Check if on floor
		if inverseGravity:
			var aboveTileData: TileData = tileMapLayer1.get_cell_tile_data(tileMapLayer1.local_to_map(global_position) + Vector2i(0, -1))
			isOnFloor = aboveTileData && !aboveTileData.get_custom_data("walkable")
		else:
			var belowTileData: TileData = tileMapLayer1.get_cell_tile_data(tileMapLayer1.local_to_map(global_position) + Vector2i(0, 1))
...
func move_x(direction: int):
	...
	oldPosition.x = tileMapLayer1.map_to_local(currentTile).x
	newPosition.x = tileMapLayer1.map_to_local(targetTile).x
...
func move_y(direction: int):
	...
	oldPosition.y = tileMapLayer1.map_to_local(currentTile).y
	newPosition.y = tileMapLayer1.map_to_local(targetTile).y
...
func update_position():
	...
	for child in tileMap.get_children():
		if child is TileMapLayer:
			var tileData: TileData = child.get_cell_tile_data(tileMapLayer1.local_to_map(newPosition))
